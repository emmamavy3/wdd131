...
// 2
function searchList(list, querry) {
  function searchCallback(string) {
    return string.toLowerCase().includes(querry.toLowerCase());
  }
...
// 3
function searchList2(list, querry) {
  function searchCallback(item) {
    return (
      ...
      item.tags.find((tag) => tag.toLowerCase().includes(querry.toLowerCase()))
    );
  }
  return list.filter(searchCallback);
}
